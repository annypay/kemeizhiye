...
if(coverPages.length){console.error('[dbg] 封面文本条数='+coverPages[0].texts.length);coverPages[0].texts.forEach((t,i)=>console.error('  ['+i+'] '+JSON.stringify(t)));}
...
for (const key of ['20260915 现场图集', '2026年09月15日', `${files.length} 张`, '现场施工情况照片汇编']) {
  if (!coverText.includes(key)) problems.push(`封面缺少「${key}」`);
}
